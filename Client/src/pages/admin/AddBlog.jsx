...
const generateContent = async () => {
    ...
    try {
      setGenerating(true)
      const { data } = await axios.post('/api/blog/generate', { title })
      if (data.success) {
    ...
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow-rounded'>
        ...
        <input type="text" placeholder='Type here' required className='w-full max-w-lg mt-2 p-2 border border-gray-300 outline-none rounded' onChange={e=> setSubTitle(e.target.value)} value = {subTitle}/>
          ...
          <button type='button' onClick={generateContent} disabled = {generating} className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer'>{generating ? 'Generating...' : 'Generate with AI'}</button>
          </div>

          <p className='mt-4'>Blog Catogory</p>
